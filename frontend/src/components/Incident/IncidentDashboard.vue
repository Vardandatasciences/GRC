        ...
        <div class="kpi-card expanded-card">
            ...
            <svg viewBox="0 0 100 100" class="donut-chart-svg">
              ...
              <template v-if="kpiData.incidentOrigins && kpiData.incidentOrigins.length > 0">
                <template v-for="(item, index) in kpiData.incidentOrigins" :key="'origin-' + index">
                  ...
                  <circle r="30" cx="50" cy="50" fill="transparent"
              ...
              <text x="50" y="47" text-anchor="middle" font-size="8" font-weight="bold">Total</text>
              <text x="50" y="58" text-anchor="middle" font-size="12" font-weight="bold">{{ incidentCounts.total }}</text>
            </svg>
          ...
          <div class="chart-legend compact-legend">
            <div v-for="(item, index) in kpiData.incidentOrigins" :key="'legend-' + index" 
                 class="legend-item compact-legend-item">
              <span class="legend-color" :style="{ backgroundColor: getOriginColor(item.origin) }"></span>
              <span class="legend-compact-text">{{ item.origin }} ({{ item.count }})</span>
            </div>
          </div>
        </div>
      </div>
