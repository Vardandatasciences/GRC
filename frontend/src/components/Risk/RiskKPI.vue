    ...
    <div class="kpi-grid">
      ...
      <div class="kpi-box" v-show="isVisible('identification-rate')">
        <div class="kpi-header">
          ...
          <div class="kpi-period-selector">
            <select v-model="identificationPeriod" @change="fetchIdentificationData">
              <option value="30days">Last 30 days</option>
              <option value="90days">Last 90 days</option>
              <option value="6months">Last 6 months</option>
              <option value="1year">Last 1 year</option>
        ...
        <div class="kpi-number">
          {{ identificationData.current || kpiData.identificationRate || '88' }}<span class="percent">%</span>
          <span class="trend-indicator" :class="{ 'up': (identificationData.percentageChange > 0), 'down': (identificationData.percentageChange < 0) }">
            {{ identificationData.percentageChange > 0 ? '↑' : '↓' }} {{ Math.abs(identificationData.percentageChange || 0) }}%
          </span>
        </div>
        <div class="identification-metrics">
          <div class="metric-item">
            <span class="metric-label">Avg. Daily</span>
            <span class="metric-value">{{ identificationData.dailyAverage || '4.2' }} risks/day</span>
          </div>
          <div class="metric-item" v-if="identificationData.totalRisksIdentified">
            <span class="metric-label">Total</span>
            <span class="metric-value">{{ identificationData.totalRisksIdentified }} risks</span>
          </div>
        </div>
        <div class="sparkline-chart">
          <div class="line-chart" v-if="identificationData.trendData && identificationData.trendData.length > 0">
            <svg width="100%" height="100%" preserveAspectRatio="none" viewBox="0 0 100 30">
              <path :d="generateSmoothCurvePath(identificationData.trendData, identificationData.minValue, identificationData.maxValue)" class="identification-path"></path>
              <path :d="generateSmoothAreaPath(identificationData.trendData, identificationData.minValue, identificationData.maxValue)" class="identification-area"></path>
            </svg>
            <div v-for="(value, index) in identificationData.trendData" :key="'id-point-'+index"
                 class="data-point identification-point"
                 :style="{ left: `${(index / (identificationData.trendData.length - 1)) * 100}%`, 
                           top: `${calculatePointPosition(value, identificationData.minValue, identificationData.maxValue)}%` }">
            </div>
            <div v-for="(value, index) in identificationData.trendData" :key="'id-tooltip-'+index"
                 class="data-tooltip"
                 :style="{ left: `${(index / (identificationData.trendData.length - 1)) * 100}%`, top: `${calculatePointPosition(value, identificationData.minValue, identificationData.maxValue)}%` }">
              {{ identificationData.months[index] }}: {{ value }}%
            </div>
          </div>
          <div class="month-labels" v-if="identificationData.months">
            <span v-for="(month, index) in identificationData.months" :key="'id-month-'+index">{{ month }}</span>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async fetchExposureScoreData() {
      try {
        const response = await fetch('/api/risk/exposure-score/');
        if (response.ok) {
