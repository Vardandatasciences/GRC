...
export default {
  ...
  computed: {
    groupedCompliances() {
      ...
      // Group compliances by their Identifier
      const groups = {};
      ...
      Object.values(groups).forEach(group => {
        group.sort((a, b) => {
          // Try to parse as semantic version (e.g., 1.0.1)
          const partsA = a.ComplianceVersion.split('.').map(n => parseInt(n, 10));
          const partsB = b.ComplianceVersion.split('.').map(n => parseInt(n, 10));
  ...
  methods: {
    ...
    async loadCompliances() {
      ...
      try {
        ...
        if (response.data.success) {
          // The API now returns data already grouped by Identifier
          // We just need to flatten it for our component's data structure
    ...
    async toggleActiveStatus(compliance) {
      ...
      try {
        ...
        if (response.data.success) {
          // After successful toggle, reload all compliances to get updated status
