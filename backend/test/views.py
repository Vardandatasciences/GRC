...
# Disable external logging service
LOGGING_SERVICE_URL = None  # Change from "http://localhost:4000/api/logs" to None
...
class RiskViewSet(viewsets.ModelViewSet):
    ...
    def list(self, request):
        ...
        print(f"Found {risks.count()} risks in database")  # Debug print
            ...
            if field in mutable_data and mutable_data[field]:
                try:
                    if isinstance(mutable_data[field], str):
                        date_value = datetime.strptime(mutable_data[field], "%Y-%m-%d %H:%M:%S")
                        mutable_data[field] = timezone.make_aware(date_value)
                except (ValueError, TypeError) as e:
                    print(f"Error parsing date for {field}: {e}")
    ...
    try:
        ...
        if due_date:
            ...
            try:
                ...
                risk_instance.MitigationDueDate = due_date_obj
            except ValueError:
