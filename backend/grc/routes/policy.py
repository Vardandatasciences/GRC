    ...
    if request.method == 'GET':
        ...
        for framework in frameworks:
            framework_data.append({
                ...
                'Reviewer': getattr(framework, 'Reviewer', None)  # Safely get Reviewer attribute
            })
...
def policy_list(request):
    ...
    # Use a simpler serialization approach to avoid the serializer error
    policy_data = []
    for policy in policies:
        # Get framework details
        framework = policy.FrameworkId
        framework_name = framework.FrameworkName if framework else "Unknown"
        framework_category = framework.Category if framework else ""
...
def get_framework_details(request, framework_id):
    """
    API endpoint for detailed framework information including policies and subpolicies
    """
    try:
        # Get framework by ID
        framework = get_object_or_404(Framework, FrameworkId=framework_id)
        ...
        # Prepare policies data with subpolicies
        policies_data = []
        for policy in policies:
        ...
        print(f"Error getting framework details: {str(e)}")
        return Response({
            'error': str(e)
        }, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
def get_policy_kpis(request):
    try:
        ...
        # Check if AcknowledgementCount field exists in the Policy model
        policies = Policy.objects.filter(ActiveInactive='Active')
        
        # Check if the model has AcknowledgementCount field
        if hasattr(Policy, 'AcknowledgementCount'):
            ...
            top_acknowledged_policies = [
                {
                    'policy_id': policy.PolicyId,
                    'policy_name': policy.PolicyName,
                    'acknowledged_count': policy.AcknowledgementCount or 0,
                    'total_users': total_users,
                    'acknowledgement_rate': round(float(getattr(policy, 'acknowledgement_rate', 0)), 1)
                }
                for policy in policies
            ]
        else:
            # If AcknowledgementCount doesn't exist, return empty list
            top_acknowledged_policies = []
