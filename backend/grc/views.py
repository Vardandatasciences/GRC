    ...
    try:
        ...
        for group in sorted_groups:
            ...
            for compliance in group:
                ...
                group_data.append(serializer.data)
    ...
    try:
        ...
        for compliance in compliances:
            compliances_data.append({
                ...
                'ManualAutomatic': compliance.ManualAutomatic,
                'CreatedByName': compliance.CreatedByName,
                'CreatedByDate': compliance.CreatedByDate.strftime('%Y-%m-%d %H:%M:%S') if compliance.CreatedByDate else '',
                'ComplianceVersion': compliance.ComplianceVersion,
                'Identifier': compliance.Identifier,
                'ActiveInactive': compliance.ActiveInactive,
                'IsRisk': compliance.IsRisk,
                'SubPolicyName': compliance.SubPolicy.SubPolicyName,
                'PolicyName': compliance.SubPolicy.Policy.PolicyName,
                'FrameworkName': compliance.SubPolicy.Policy.Framework.FrameworkName
            })
        ...
        print(f"Error fetching subpolicy compliances: {str(e)}")
        return Response({
            'success': False,
            'message': str(e)
        }, status=500)
...
def get_framework_compliances(request, framework_id):
    ...
    try:
        ...
        compliances = Compliance.objects.filter(
            SubPolicy__Policy__Framework=framework
        ).select_related('SubPolicy', 'SubPolicy__Policy')
...
def export_compliances(request, export_format, item_type=None, item_id=None):
    try:
        ...
        # Handle legacy URL pattern (without item_type and item_id)
        ...
        print(f"Using item_type: {item_type}, item_id: {item_id}")
        
        # Validate format
        valid_formats = ['xlsx', 'csv', 'pdf', 'json', 'xml']
        ...
        # Get compliances based on the item type
        if item_type == 'framework':
            try:
                ...
                return Response({
                    'success': False,
                    'message': f'Framework with ID {item_id} not found'
                }, status=404)
            ...
            try:
                ...
                return Response({
                    'success': False,
                    'message': f'Policy with ID {item_id} not found'
                }, status=404)
            ...
            try:
                ...
                return Response({
                    'success': False,
                    'message': f'Subpolicy with ID {item_id} not found'
                }, status=404)
        else:
            return Response({
                'success': False,
                'message': 'Invalid item type. Must be framework, policy, or subpolicy'
            }, status=400)
        ...
        for compliance in compliances:
            export_data.append({
                ...
                'Status': compliance.Status,
                'Criticality': compliance.Criticality,
                'Maturity Level': compliance.MaturityLevel,
                'Type': compliance.MandatoryOptional,
                'Implementation': compliance.ManualAutomatic,
                'Created By': compliance.CreatedByName,
                'Created Date': compliance.CreatedByDate.strftime('%Y-%m-%d %H:%M:%S') if compliance.CreatedByDate else '',
                'Version': compliance.ComplianceVersion,
                'Identifier': compliance.Identifier,
                'Active/Inactive': compliance.ActiveInactive,
                'Is Risk': 'Yes' if compliance.IsRisk else 'No',
                'SubPolicy': compliance.SubPolicy.SubPolicyName,
                'Policy': compliance.SubPolicy.Policy.PolicyName,
                'Framework': compliance.SubPolicy.Policy.Framework.FrameworkName
            })
        ...
        filename = f'compliances_{prefix}_{name}_{timestamp}.{export_format}'

        try:
            # Export based on format
            if export_format == 'xlsx':
                output = export_to_excel(export_data)
                content_type = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
            elif export_format == 'csv':
                output = export_to_csv(export_data)
                content_type = 'text/csv'
            elif export_format == 'pdf':
                output = export_to_pdf(export_data)
                content_type = 'application/pdf'
            elif export_format == 'json':
                output = export_to_json(export_data)
                content_type = 'application/json'
            else:  # xml
                output = export_to_xml(export_data)
            ...
            print(f"Error during export generation: {str(export_error)}")
            return Response({
                'success': False,
                'message': f'Error generating export: {str(export_error)}'
            }, status=500)
