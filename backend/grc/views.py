...
from .models import PolicyApproval  # Make sure this model exists
...
def get_latest_policy_approval(request, policy_id):
    """
    Get the latest policy approval for a given policy ID regardless of version
    """
    try:
        # Get the latest approval record for this policy
        latest_approval = PolicyApproval.objects.filter(
            PolicyId=policy_id
        ).order_by('-ApprovedDate').first()
        
        if not latest_approval:
            return Response({"error": "No approval records found for this policy"}, 
                           status=status.HTTP_404_NOT_FOUND)
        
        # Return the serialized data
        return Response({
            "ApprovalId": latest_approval.ApprovalId,
            "PolicyId": latest_approval.PolicyId,
            ...
            "ApprovedDate": latest_approval.ApprovedDate,
...
def get_latest_policy_approval_by_role(request, policy_id, role):
    """
    Get the latest policy approval for a given policy ID and role (user or reviewer)
    role parameter should be 'user' or 'reviewer'
    """
    try:
        # Determine version prefix based on role
        version_prefix = 'u' if role.lower() == 'user' else 'r'
        
        # Filter approvals for this policy with the correct version prefix
        approvals = PolicyApproval.objects.filter(
            PolicyId=policy_id,
            Version__istartswith=version_prefix
        ).order_by('-ApprovedDate')
        
        if not approvals.exists():
            return Response({"error": f"No {role} approval records found for this policy"}, 
                           status=status.HTTP_404_NOT_FOUND)
...
def incident_mttd(request):
    ...
    try:
        ...
        # Instead of doing complex calculations in Django ORM, do it in Python
        ...
        if all_incidents:
            ...
            for incident in all_incidents:
                ...
                minutes = diff_seconds / 60  # Convert to minutes instead of hours
        ...
        # Create placeholder trend data (with minutes)
        ...
        for month in months:
            trend_data.append({
                'month': month,
                'minutes': mttd_value,  # Store as minutes
                'count': 0
            })
        
        # Prepare response data with minutes as the unit
        response_data = {
            'value': mttd_value,
            'unit': 'minutes',  # Change unit to minutes
            'change_percentage': 0,  # Default to 0% change
            'trend': trend_data
        }
        ...
        # Log the error
        import logging
        logging.error(f"Error calculating MTTD: {str(e)}")
        ...
        response_data = {
            ...
            'unit': 'minutes',  # Change unit to minutes
            'change_percentage': 0,
            'trend': [{'month': m, 'minutes': 0, 'count': 0} for m in ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun']]
        }
    
    return JsonResponse(response_data)
def incident_mttr(request):
    """Calculate Mean Time to Respond (MTTR) - time from identification to first mitigation"""
    ...
    # Filter only incidents that have both IdentifiedAt and FirstMitigationAt
    incidents = incidents.filter(IdentifiedAt__isnull=False, FirstMitigationAt__isnull=False)
    
    # Calculate time difference
    time_diff = ExpressionWrapper(
        F('FirstMitigationAt') - F('IdentifiedAt'),
...
def incident_mttc(request):
    """Calculate Mean Time to Contain (MTTC) - time from identification to containment"""
    timeframe = request.GET.get('timeRange', 'all')
    
    # Filter incidents based on timeframe
    incidents = filter_incidents_by_timeframe(timeframe)
    
    # Filter only incidents that have both IdentifiedAt and ContainedAt
    incidents = incidents.filter(IdentifiedAt__isnull=False, ContainedAt__isnull=False)
    
    # Calculate time difference
    time_diff = ExpressionWrapper(
        F('ContainedAt') - F('IdentifiedAt'),
        output_field=fields.DurationField()
    )
    
    avg_time = incidents.annotate(containment_time=time_diff).aggregate(
        avg_time=Avg('containment_time')
    )
    ...
    for i in range(7, 0, -1):
        ...
        day_incidents = incidents.filter(IdentifiedAt__date=date)
        day_avg = day_incidents.annotate(containment_time=time_diff).aggregate(
            avg_time=Avg('containment_time')
        )
...
def incident_volume(request):
    """Get the total volume of incidents over time"""
    timeframe = request.GET.get('timeRange', 'all')
    ...
    for i in range(13, -1, -1):
        date = today - timedelta(days=i)
        count = incidents.filter(CreatedAt__date=date).count()
        daily_data.append({
            'date': date.strftime('%Y-%m-%d'),
            'count': count
        })
...
def escalation_rate(request):
    """Get the percentage of incidents escalated from audit vs manually raised"""
    timeframe = request.GET.get('timeRange', 'all')
    ...
    # Count incidents by origin
    audit_count = incidents.filter(Origin='Audit').count()
    manual_count = total - audit_count
    
    # Calculate percentages
    audit_percent = (audit_count / total) * 100
    manual_percent = (manual_count / total) * 100
    
    return JsonResponse({
        'value': round(audit_percent, 0),
        'audit': round(audit_percent, 0),
        'manual': round(manual_percent, 0)
    })

def repeat_rate(request):
    """Get the percentage of incidents that are repeats/recurrences"""
    timeframe = request.GET.get('timeRange', 'all')
    ...
    # Count repeat incidents (those with RelatedIncidentId or IsReopened=True)
    repeat_count = incidents.filter(
        Q(RelatedIncidentId__isnull=False) | Q(IsReopened=True)
    ).count()
    
    new_count = total - repeat_count
    
    # Calculate percentages
    repeat_percent = (repeat_count / total) * 100
    new_percent = (new_count / total) * 100
    
    return JsonResponse({
        'value': round(repeat_percent, 0),
        'new': round(new_percent, 0),
        'repeat': round(repeat_percent, 0)
    })
