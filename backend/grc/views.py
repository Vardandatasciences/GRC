...
def allocate_policy(request):
    ...
    try:
        ...
        if serializer.is_valid():
            ...
            if selected_reports:
                reports_dict = {f"id_{i+1}": report_id for i, report_id in enumerate(selected_reports)}
...
def get_my_audits(request):
    ...
    try:
        ...
        for audit in audits:
            ...
            print(reports_data,'------------------------------------')
            audit['report_available'] = bool(reports_data and reports_data != 'null' and reports_data != '[]' and reports_data != '{}')
...
def update_audit_finding(request, compliance_id):
    ...
    try:
        ...
        if 'criticality' in request.data:
            finding.MajorMinor = '1' if request.data['criticality'] == 'Major' else '0'
...
def save_review_progress(request, audit_id):
    ...
    try:
        ...
        if has_rejected:
            ...
            # Update audit status back to "Work In Progress"
            try:
                audit = Audit.objects.get(AuditId=audit_id)
                if audit.Status == 'Under review':
...
@api_view(['POST'])
def save_reviewer_version(request, audit_id):
    """
...
def debug_rejection_process(request, audit_id):
    ...
    try:
        ...
        # Get the latest reviewer version (R-prefix) that contains the rejection
        latest_reviewer_data = None
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                SELECT Version, ExtractedInfo, Date 
                FROM audit_version 
                WHERE AuditId = %s AND Version LIKE 'R%' AND (ApprovedRejected = 'Rejected' OR ApprovedRejected = 'R')
                ORDER BY Date DESC LIMIT 1
                """,
                [audit_id]
            )
            ...
            if row:
                ...
                print(f"DEBUG: Found rejection version: {rejection_version} from {rejection_date}")
            else:
                print(f"DEBUG: No explicit rejection version found, checking all reviewer versions")
                
                # Try finding any reviewer version
                cursor.execute(
                    """
                    SELECT Version, ExtractedInfo, Date 
                    FROM audit_version 
                ...
                if row:
                    ...
                    if isinstance(rejection_json, str):
                        try:
                            ...
                            print(f"ERROR parsing reviewer JSON: {str(e)}")
                            latest_reviewer_data = {"error": "Could not parse JSON"}
                    else:
                        latest_reviewer_data = rejection_json
...
@api_view(['GET'])
def debug_version_json(request, audit_id, version_id=None):
    """Debug endpoint to print the JSON data for a specific version or the latest version"""
    try:
        print(f"=== DEBUG: Version JSON data for audit_id={audit_id}, version_id={version_id} ===")
        
        # Get the audit record
        audit = Audit.objects.get(AuditId=audit_id)
        
        # Construct query based on whether version_id was provided
        query = "SELECT Version, ExtractedInfo, Date FROM audit_version WHERE AuditId = %s"
        params = [audit_id]
        ...
        with connection.cursor() as cursor:
            ...
            if not row:
                if version_id:
                    return Response({"error": f"Version {version_id} not found for audit {audit_id}"}, 
                                  status=status.HTTP_404_NOT_FOUND)
                else:
                    return Response({"error": f"No versions found for audit {audit_id}"}, 
                                  status=status.HTTP_404_NOT_FOUND)
            ...
            if isinstance(json_data, str):
                try:
                    ...
                    parsed_data = {"error": "Could not parse JSON"}
            else:
                parsed_data = json_data
            
            # Format the JSON nicely for inspection
            formatted_json = json.dumps(parsed_data, indent=2)
            print(f"DEBUG: JSON data for version {version}:\n{formatted_json}")
            
            return Response({
                "audit_id": audit_id,
                "version": version,
                "date": date.strftime('%Y-%m-%d %H:%M:%S') if date else None,
                "json_size": len(formatted_json),
        ...
        print(f"ERROR in debug_version_json: {str(e)}")
        import traceback
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        
@api_view(['GET'])
def debug_data_flow(request, audit_id):
    """Debug endpoint to track data flow between auditor and reviewer"""
    try:
        print(f"=== DEBUG: Data flow debug for audit_id={audit_id} ===")
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT Version, ExtractedInfo, Date, UserId 
                FROM audit_version 
                WHERE AuditId = %s 
                ORDER BY Date ASC
                """,
            ...
            for row in cursor.fetchall():
                ...
                # Just extract metadata to avoid huge response
                metadata = None
                if isinstance(json_data, str):
                    try:
                        ...
                        metadata = {"error": "Could not parse JSON"}
                else:
                    if "__metadata__" in json_data:
                        metadata = json_data["__metadata__"]
        ...
        # Track status changes in audit table
        status_timeline = []
        ...
        audit_info = {
            ...
            "Current ReviewStatus": audit.ReviewStatus,
            "Auditor": f"{audit.Auditor.UserName} (ID: {audit.Auditor.UserId})",
            "Reviewer": f"{audit.Reviewer.UserName} (ID: {audit.Reviewer.UserId})" if audit.Reviewer else "None",
            "Assignee": f"{audit.Assignee.UserName} (ID: {audit.Assignee.UserId})",
            "AssignedDate": audit.AssignedDate.strftime('%Y-%m-%d %H:%M:%S') if audit.AssignedDate else None,
            "CompletionDate": audit.CompletionDate.strftime('%Y-%m-%d %H:%M:%S') if audit.CompletionDate else None,
            "ReviewDate": audit.ReviewDate.strftime('%Y-%m-%d %H:%M:%S') if audit.ReviewDate else None
        }
        
        # Get state of latest finding updates
        ...
        with connection.cursor() as cursor:
            ...
            for row in cursor.fetchall():
                ...
                findings_update_status.append(finding_data)
        
        # Generate data flow timeline
        data_flow = []
        
        # Add version events
        for v in versions:
            data_flow.append({
                "event": f"Version {v['version_id']} created",
                "type": v['version_type'],
                "date": v['date'],
                "user": v['user_name'],
                "details": v['metadata'] if v['metadata'] else {}
            })
        
        # Add finding update events
        for f in findings_update_status:
            if f.get('CheckedDate'):
                data_flow.append({
                    "event": f"Compliance {f['ComplianceId']} updated by auditor",
                    "type": "Finding Update",
                    "date": f['CheckedDate'],
                    "details": {
            ...
            if f.get('ReviewDate'):
                data_flow.append({
                    "event": f"Compliance {f['ComplianceId']} reviewed",
                    "type": "Finding Review",
                    "date": f['ReviewDate'],
                    "details": {
                        "compliance_id": f['ComplianceId'],
                        "rejected": f['ReviewRejected'],
                        "review_status": f['ReviewStatus'],
                        "comments": f['ReviewComments']
                    }
                })
        
        # Sort data flow by date
        data_flow.sort(key=lambda x: x['date'] if x.get('date') else '0000-00-00')
        
        return Response({
            "audit_info": audit_info,
            "versions_count": len(versions),
            "versions": versions,
            "findings_updates": findings_update_status,
            "data_flow": data_flow
        }, status=status.HTTP_200_OK)
        ...
        print(f"ERROR in debug_data_flow: {str(e)}")
        import traceback
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        
@api_view(['GET'])
def debug_reviewer_comments(request, audit_id):
    """Debug endpoint to extract and display reviewer comments from the JSON data"""
    try:
        print(f"=== DEBUG: Reviewer comments for audit_id={audit_id} ===")
        ...
        with connection.cursor() as cursor:
            ...
            if row:
                ...
                return Response({"message": f"No reviewer version found for audit {audit_id}"}, 
                               status=status.HTTP_404_NOT_FOUND)
        
        # Extract all reviewer comments from the JSON
        reviewer_comments = {}
        overall_comments = reviewer_data.get('overall_comments', '')
        
        # Extract metadata
        metadata = None
        if '__metadata__' in reviewer_data:
            metadata = reviewer_data['__metadata__']
        ...
        for key, value in reviewer_data.items():
            ...
            if isinstance(value, dict):
                # Get the compliance description if available
                description = value.get('description', 'Unknown')
                
                # Get reviewer comments using various possible field names
                comments = None
                if 'review_comments' in value:
                    comments = value['review_comments']
                elif 'comments' in value:
                    comments = value['comments']
                
                # Get reviewer accept/reject status
                status = None
                if 'review_status' in value:
                    status = value['review_status']
                elif 'accept_reject' in value:
                    accept_reject = value['accept_reject']
                    if accept_reject == "1":
                        status = "Accept"
                ...
                # Only include items with comments or explicit status
                if comments or status:
        ...
        # Also get comments from the audit_findings table
        findings_comments = []
        ...
        with connection.cursor() as cursor:
            cursor.execute(
                ...
                WHERE 
                    af.AuditId = %s AND 
                    (af.ReviewComments IS NOT NULL AND af.ReviewComments != '')
                """,
                [audit_id]
            )
            ...
            for row in cursor.fetchall():
                ...
                findings_comments.append(finding_data)
        
        # Print the full JSON data for the reviewer version
        formatted_json = json.dumps(reviewer_data, indent=2)
        print(f"DEBUG: Full JSON data for reviewer version {reviewer_version}:\n{formatted_json}")
        ...
        return Response({
            ...
            'overall_comments': overall_comments,
            'metadata': metadata,
            'reviewer_comments_count': len(reviewer_comments),
            'reviewer_comments': reviewer_comments,
            'findings_comments_count': len(findings_comments),
            'findings_comments': findings_comments
        }, status=status.HTTP_200_OK)
    
    except Audit.DoesNotExist:
        return Response({"error": f"Audit with ID {audit_id} not found"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        print(f"ERROR in debug_reviewer_comments: {str(e)}")
        import traceback
        traceback.print_exc()
        return Response({"error": str(e)}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
        ...
        with connection.cursor() as cursor:
            if is_new_version:
                ...
                cursor.execute(
                    ...
                    VALUES (%s, %s, %s, %s, NOW())
                    """,
                    [audit_id, latest_version, json_data, user_id]
                )
