...
BUCKET_NAME = os.environ.get('S3_BUCKET', 'grc-files-vardaan')
...
def save_export_record(export_data):
    """Save export record using the ExportedFile model"""
    export_file = ExportedFile(
        export_data=json.dumps(export_data.get('export_data')),
        file_type=export_data.get('file_type'),
        user_id=export_data.get('user_id'),
        s3_url=export_data.get('s3_url', ''),  # S3 URL initially empty
...
def update_export_status(export_id, status, error=None):
    ...
    try:
        export_file = ExportedFile.objects.get(id=export_id)
        export_file.status = status
        export_file.error = error
        
        if status == 'completed':
            export_file.completed_at = datetime.datetime.now()
            
        export_file.save()
    except ExportedFile.DoesNotExist:
        print(f"Export record with ID {export_id} not found")
...
def update_export_metadata(export_id, metadata):
    ...
    try:
        ...
        existing_metadata = json.loads(export_file.metadata or '{}')
        updated_metadata = {**existing_metadata, **metadata}
        
        export_file.metadata = json.dumps(updated_metadata)
        export_file.save()
    except ExportedFile.DoesNotExist:
        print(f"Export record with ID {export_id} not found")
